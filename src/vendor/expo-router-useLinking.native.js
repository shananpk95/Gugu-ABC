...
const extractPathFromURL_1 = require("expo-router/build/fork/extractPathFromURL");
const native_1 = require("expo-router/build/react-navigation/native");
...
function useLinking(ref, { enabled = true, prefixes, filter, config, getInitialURL = () => getInitialURLWithTimeout(), subscribe = (listener) => {
    ...
    // Defer Android getInitialURL().then → setState until after NavigationContainer mounts.
    // Workaround for https://github.com/expo/expo/issues/50282
    const isMountedRef = (0, react_1.useRef)(false);
    const pendingUnhandledLinkRef = (0, react_1.useRef)(undefined);
    const onUnhandledLinkingRef = (0, react_1.useRef)(onUnhandledLinking);
    onUnhandledLinkingRef.current = onUnhandledLinking;
    (0, react_1.useEffect)(() => {
        isMountedRef.current = true;
        if (pendingUnhandledLinkRef.current !== undefined) {
            onUnhandledLinkingRef.current(pendingUnhandledLinkRef.current);
    ...
    const reportUnhandledLinking = (0, react_1.useCallback)((url) => {
        const path = (0, extractPathFromURL_1.extractExpoPathFromURL)(prefixes, url);
        if (isMountedRef.current) {
            onUnhandledLinkingRef.current(path);
        }
        else {
    ...
    const getInitialState = (0, react_1.useCallback)(() => {
        ...
        return thenable;
    }, [getStateFromURL, onUnhandledLinking, prefixes, reportUnhandledLinking]);
    (0, react_1.useEffect)(() => {
        const listener = (url) => {
            if (!enabled) {
                return;
